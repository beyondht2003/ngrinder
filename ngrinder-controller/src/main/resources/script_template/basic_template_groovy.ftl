...
<#if options??>
    ...
    <#assign assertBodies = options["assertBodies"]>
</#if>
class TestRunner {
...
<#else>
	...
	public void test(){
		...
		<#--}-->
		assertThat(result.statusCode, is(200))
	<#if assertBodies?? && assertBodies?size != 0>
		<#list assertBodies as assertBody>
		assertThat(result.getText(), containsString("${assertBody["value"]?j_string?replace("$", "\\$")}"))
		</#list>
	</#if>
	}
}
