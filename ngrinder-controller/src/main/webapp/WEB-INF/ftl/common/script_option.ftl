...
<hr class="small">
<div id="assertRow" class="row form-horizontal">
	<div class="option-title span1"><h5><@spring.message "script.option.resAssert.title"/></h5></div>
	<div id="params" class="option-data span9">
		<i title="Add assert" id="addAssertBtn" class="icon-plus pointer-cursor"></i>
	</div>
</div>
...
<script type="text/javascript">
	...
	$(document).ready(function () {
		...
		$("#addAssertBtn").click(function () {
			var assertBody = new SingleValue("assertBody");
			options.assertBodies.push(assertBody);
			$("#addAssertBtn").before(assertBody.getHtml());
			console.log(assertBody)
		});
	});
	...
	var options = {
		...
		toJson: function (method) {
			...
			this._appendAssertBodyJson(json);
		...
		_appendParamJson: function (json) {
			if ($("#bodyRow").hasClass("hide")) {
				console.log("yes bodyRow hasClass hide");
				for (var i = 0; i < this.params.length; i++) {
					json.params.push({
						name: this.params[i].getName(),
						value: this.params[i].getValue()
					});
					console.log(this.params[i].getValue())
				}
			} else {
				console.log("no bodyRow hasClass hide");
				json.body = $("#reqBodyText").val();
			}
		},
		_appendAssertBodyJson: function (json) {
			for (var i = 0; i < this.assertBodies.length; i++) {
				json.assertBodies.push({
					value: this.assertBodies[i].getValue()
				});
			}
		},
	...
	Pair.prototype = {
		getHtml: function () {
			var deleteBtn = $("<i>", {
				class: "icon-minus pointer-cursor",
				title: "Delete",
	...
	function SingleValue(cls) {
		this.root = $("<div>", {class: cls});
		this.value = $("<input>", {class: "input value span2-3", placeholder: "value", type: "text"});
		this.value.appendTo(this.root);
	}

	SingleValue.prototype = {
		getHtml: function () {
			var deleteBtn = $("<i>", {
				class: "icon-minus pointer-cursor",
				title: "Delete",
